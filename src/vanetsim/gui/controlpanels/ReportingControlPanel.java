...
public final class ReportingControlPanel extends JPanel implements ActionListener, ItemListener{
	...
	private final void updateStatistics(){
		...
		double[] chromeTime = new double[50];
		int c = 0;
		for(i = 0; i < regions.length; ++i){
			for(j = 0; j < regions[i].length; ++j){
				...
				for(k = 0; k < vehicles.length; ++k){
					vehicle = vehicles[k];
					c = vehicle.getChromosome();
					if(vehicle.getTotalTravelTime() > 0){
						++travelledVehicles;
						travelDistance += vehicle.getTotalTravelDistance();
						travelTime += vehicle.getTotalTravelTime();
						chromeTime[c] =travelTime;
						/** send for fitness function */
					//	if(travelTime > 500) {

					//	}
						System.err.println("chromeTime: " + chromeTime[c]/5/1000);
					}
					if(vehicle.isActive()){
						++activeVehicles;

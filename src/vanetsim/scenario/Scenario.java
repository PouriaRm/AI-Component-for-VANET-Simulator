...
public final class Scenario{
	...
	public void load(File file, boolean zip){
		...
		try{
			...
			initNewScenario();
			String type;
			int x, y, time, maxSpeed, vehicleLength, maxCommDistance, direction, lanes, braking_rate, acceleration_rate, timeDistance, politeness, color, mixX, mixY, mixRadius, wifiX, wifiY, wifiRadius, chromosome;
						...
						while (vehiclesCrsr.getNext() != null){
							if(vehiclesCrsr.getLocalName().toLowerCase().equals("vehicle")){ //$NON-NLS-1$
								...
								while (vehicleCrsr.getNext() != null){
									...
									} else if(vehicleCrsr.getLocalName().toLowerCase().equals("chromosome")){ //$NON-NLS-1$
										try{
											chromosome = Integer.parseInt(vehicleCrsr.collectDescendantText(false));
										} catch (Exception e) {}
